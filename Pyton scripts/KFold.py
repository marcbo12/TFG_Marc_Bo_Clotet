...
path_train = "/home/usuaris/imatge/marc.bo/proj/headpose/paths/splits/train_2_rgb.txt"
path_test =  "/home/usuaris/imatge/marc.bo/proj/headpose/paths/splits/test_2_rgb.txt"
...
writer = SummaryWriter('runs/KFold/' + run)
...
def train(epoch, model_yaw, model_pitch, optimizer_yaw, optimizer_pitch, criterion, loader, fold):
    model_yaw.train()
    model_pitch.train()
    loss_pitch_tot = 0
    loss_yaw_tot = 0
    ...
    writer.add_scalar('Loss/train_Pitch {}'.format(fold), loss_pitch_tot/i, epoch)
    writer.add_scalar('Loss/train_Yaw {}'.format(fold), loss_yaw_tot/i, epoch)
    print('Train Epoch: {} Loss: Horizontal {:.6f} Vertical: {:.6f} Fold: {}'.format(epoch + 1, loss_pitch_tot / (i),
                                                                            loss_yaw_tot / (i), fold))
    return loss_pitch_tot / i, loss_yaw_tot / i

def test(model_yaw, model_pitch, criterion, loader, fold):
    ...
    writer.add_scalar('Loss/test_Pitch Fold {}'.format(fold), loss_pitch_tot/i, epoch)
    writer.add_scalar('Loss/test_Yaw Fold {}'.format(fold), loss_yaw_tot/i, epoch)
    print('\nTest set: Average loss: Pitch {:.4f} Yaw {:.4f} Fold: {} \n'.format(loss_pitch_tot/(i), loss_yaw_tot/(i),fold))
    return loss_pitch_tot/i, loss_yaw_tot/i
...
for fold ,(train_index, test_index)in enumerate(kf.split(train_dataset)):
    fold_losses_Pitch_train = []
    fold_losses_Yaw_train = []
    fold_losses_Pitch_test = []
    fold_losses_Yaw_test = []
    #For each fold restart model
    model_yaw = MyModel()
    model_pitch = MyModel()
    model_yaw.cuda()
    model_pitch.cuda()
    ...
    #select each fold subset
    train_subset = torch.utils.data.Subset(train_dataset, train_index)
    test_subset = torch.utils.data.Subset(train_dataset, test_index)
    train_loader = DataLoader(dataset=train_subset, batch_size=4, shuffle=True, num_workers=4)
    test_loader = DataLoader(dataset=test_subset, batch_size=4,shuffle=False, num_workers=4)

    for epoch in range(epochs):
        losspTr, lossyTr = train(epoch, model_yaw, model_pitch, optimizer_yaw, optimizer_pitch, criterion, train_loader, fold)
        losspTs, lossyTs = test(model_yaw, model_pitch, criterion, test_loader, fold)
        fold_losses_Pitch_train.append(losspTr)
        fold_losses_Yaw_train.append(lossyTr)
        fold_losses_Pitch_test.append(losspTs)
        fold_losses_Yaw_test.append(lossyTs)
    train_losses_Pitch.append(fold_losses_Pitch_train)
    train_losses_Yaw.append(fold_losses_Yaw_train)
    test_losses_Pitch.append(fold_losses_Pitch_test)
    test_losses_Yaw.append(fold_losses_Yaw_test)


print('Finished Training')

def GenerarCorves(loss_list, name, color):
    max_loss = []
    min_loss = []
    mean_loss = []
    ...
    for epoch in range(epochs):
        ...
        min = loss_list[0][0]
        mitja = 0
        for fold_it in range(len(loss_list)):
            if loss_list[fold_it][epoch] > max:
                max = loss_list[fold_it][epoch]
            if loss_list[fold_it][epoch] < min:
                min = loss_list[fold_it][epoch]
            mitja = mitja + loss_list[fold_it][epoch]
        mitja = mitja / len(loss_list)
        mean_loss.append(mitja)
        max_loss.append(max)
        min_loss.append(min)
    ...
    fig1 = plt.figure()
    plt.plot(range(1,epochs+1), mean_loss, color = color, label = name)
    plt.fill_between(range(1,epochs+1), min_loss, max_loss, color = color, alpha = .1)
    plt.grid(b=True)
    plt.xticks(np.arange(1,20,steps= 1))
    plt.xlabel('Epochs')
    plt.ylabel(name + ' Loss')

    fig1.savefig(f'/home/usuaris/imatge/marc.bo/proj/headpose/images/{run}_{name}_loss.png', dpi = 100)
    plt.close(fig1)

GenerarCorves(train_losses_Pitch, "Pitch Train", 'red')
GenerarCorves(train_losses_Yaw, "Yaw Train", 'blue')
GenerarCorves(test_losses_Pitch, "Pitch Validation", 'green')
GenerarCorves(test_losses_Yaw, "Yaw Validation", 'cyan')

print("PLOTS DONE")

"""""
root = '/home/usuaris/imatge/marc.bo/proj/headpose/paths_d/'
...
for file1 in files:
    if file1 != 'split.py' and file1 != 'splits':
        val_dataset = AngleData(split=os.path.join(root, file1),
                                transform=transforms.Compose([transforms.Resize((640, 480), 2), transforms.ToTensor()]))
