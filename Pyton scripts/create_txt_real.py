...
root = '/mnt/gpid07/users/morros/VFOA/Trayectorias_20200313/'
saver =  '/home/usuaris/imatge/marc.bo/proj/headpose/paths_reals/'
...
for directory in dires:
    ...
    path = root+directory+'/angcomp.txt'
    path_img = root+directory+'/img/image_'
    path_depth = root+directory + '/depth/depth_'
    rec = root +directory+'/parametrosyaw.txt'
    ...
    for rootdir, dirs, files in os.walk(path_depth):
        for name in files:
            """
            if name[0:7] == 'image_':
            """
    ...
    for dire in ang2:
        join.append(path_depth + dire[0]+'.png')

    with open('paths_reals_depth/{}.txt'.format(directory),"a") as t:
