...
public class ImageSynthesis : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        path = "C:/TFG/DATASET";
        if(Female){ 
            path = path + "/Female models" +"/" + modelNumber +"/" + seqNumber +"/";
        }else{
            path = path + "/Male_models" +"/" + modelNumber + "/" + seqNumber + "/";
        }
    }
    ...
    public void Save(int frameTime, int width = -1, int height = -1) 
    {
        if (width <= 0 || height <= 0)
        {
            width = Screen.width;
        ...
        var fullname = path;
        var filenameExtension = frameTime + ".png";
        // execute as coroutine to wait for the EndOfFrame before starting capture
        StartCoroutine(
            WaitForEndOfFrameAndSave(fullname, filenameExtension, width, height));
            Debug.Log("Saved image " + frameTime);
    }
    ...
    private void Save(string filenameWithoutExtension, string filenameExtension, int width, int height)
    {
        foreach (var pass in capturePasses)
        {
            ...
            )
            {
                string fullname = "";
                if(pass.name == "_img"){
                    fullname = filenameWithoutExtension + "rgb/";
                }
                if(pass.name == "_depth"){
                    fullname = filenameWithoutExtension + "depth/";
                }

                Save(pass.camera, fullname + pass.name + filenameExtension, width, height, pass.supportsAntialiasing, pass.needsRescale);
            }
        }
    }
